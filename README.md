...
让 ChatGPT 承担适合外包的长材料阅读、发散或初审，让 Codex 负责边界、关键核验、最终决策和实施。这不是“双模型把任务各做一遍”，而是一套带成本判断、隐私 Gate 和证据闭环的协作协议。

> 当前版本为 **v1.1.0 Beta**，仅支持用户已登录的 Chrome ChatGPT 网页会话。它可能减少当前 Codex 主上下文对长源材料的摄入，但不保证减少总计算量、真实计费 token 或任务耗时。
...
- 你只说“省 token”，没有要求外部 GPT 协作；
- 你指定 Claude、Gemini 等其他 provider。v1.1 不会静默替换。
...
- 公开双语 README 共 28,261 字符；最终 Skill、模板、brief 与回复合计的“源材料摄入规避代理”为 4,718 字符，即 83.3%。该口径明确排除 19 次浏览器调用的协议内容和 3 次超时恢复，不是完整上下文或计费 token 指标。
